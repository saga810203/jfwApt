...
public class BaseTypeHeaderRPT  extends AbstractRequestParamTransfer {
        ...
        if (this.annotation.required()){
           ...
           if(Utils.isPrimitive(this.mpe.getTypeName())){
        	   if(dv!=null && dv.trim().length()!=0 && !"null".equals(dv.trim())){
                   ...
                   sb.append(" = ").append((dv==null || dv.trim().length()==0)?"null":this.annotation.defaultValue()); 
            }
            sb.append(";\r\n");
            sb.append("if(null!=param && param.length()!=0){\r\n");
    ...
    public void bulidBeanProterty() {
        ...
        if (this.frp.isRequired()){
            ...
            if(fed.length()>1) sb.append(fed.substring(1));
            sb.append("(");
            this.transferToParam();
            sb.append(");\r\n");
        }else{
