...
public class RequestBodyHandler extends BuildParamHandler.BuildParameter{
	...
	public void build(StringBuilder sb, MethodParamEntry mpe,RequestMappingCodeGenerator rmcg) throws AptException {
		RequestBody rb = rmcg.getRef().getAnnotation(RequestBody.class);
		if(Utils.isPrimitive(mpe.getTypeName())) throw new AptException(mpe.getRef(),"@RequestBody not with on primitive");
		if(rb==null) return;
       ...
       if(!parameterized){
           ...
           sb.append("org.jfw.util.json.JsonService.<").append(tn).append(">fromJson(new java.io.InputStreamReader(").append(localName)
           .append(", org.jfw.util.ConstData.UTF8),new new org.jfw.util.reflect.TypeReference<").append(tn).append(">(){}.getType() );");  
       }
